...
class AbstractAI(object):
    def __init__(self, cfg, cklist, sandbox):
        ...
        if self.do_AI_once_only and self.cklist.is_done(self.category_name, self.name):
    ...
    def do_AI(self):
        raise FatalError(
            "Internal Error: %s.do_AI method not implemented" % self.__class__.__name__
        )
...
"""
    ...
    def commit(self, branch_name, files, rev_log, review_insns=None):
        ...
        if review_insns is None:
            review_insns = PATCH_REVIEW_DEFAULT_INSNS % {"branch_name": branch_name}
        ...
        if branch_name != current_branch:
            raise FatalError(
                ...
                " - expected %(expected)s."
                % {"current": current_branch, "expected": branch_name},
                "Aborting.",
            )

        trace(
            "Checking in the following files "
            " (on branch: ${hl_sbu}%s${hl_ebu}):" % current_branch,
        ...
        for (cl_filename, cl_text) in cl_entries.items():
            self.add_changelog_entry(cl_filename=cl_filename, cl_text=cl_text)
    ...
    def email_changes(self, branch_name, subject):
        ...
        subject_prefix = "release/%s" % (
            "branch" if branch_name == self.release_branch else "HEAD"
        )
