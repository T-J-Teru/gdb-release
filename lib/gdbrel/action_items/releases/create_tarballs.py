...
class AI(AbstractReleaseCreationAI):
    ...
    def __create_tarballs(self):
        ...
        trace('Tarball created (%s), moving it to the tmp directory...'
              % tarball_name,
              '(%s)' % self.cfg['setup.tmp_dir'])
        shutil.move(os.path.join(self.sandbox.path, tarball_name),
    ...
    def __ensure_no_deleted_files(self):
        ...
        if deleted:
            ...
            raise FatalError(DELETED_FILES_ERROR
                             % {'deleted_files': deleted_str})
    ...
    def __compress_tarballs(self):
        ...
        for zip_tool in ('xz', 'gzip'):
            ...
            with open(dst, 'w') as out_fd:
                status = call([zip_tool, '-v', '-9', '-c', src],
                              stdout=out_fd, stderr=sys.stderr)
                if status != 0:
                    raise FatalError('Failed to recreate %s archive!'
                                     % zip_tool)
