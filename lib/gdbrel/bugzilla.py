...
class DumpFormat(object):
    ...
    HTML = ('<li> <a href="{bugzilla_url}/{show_bug}?id=%(id)s">'
...
def get_fixed_bugs(config, version):
    ...
    """
    request_info = ('product=gdb',
                    'resolution=FIXED',
                    'target_milestone=%s' % version,
                    'include_fields=%s' % ','.join(('id',
                                                    'component',
                                                    'summary')))
    request_url = '%s/%s/bug?%s' % (config['bugzilla.url'],
...
def dump_fixed_bugs(config, version, fmt):
    ...
    fmt = fmt.format(bugzilla_url=config['bugzilla.url'],
                     show_bug=config['bugzilla.show_bug_suburl'])
