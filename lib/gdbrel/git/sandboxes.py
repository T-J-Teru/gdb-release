...
class GitSandbox(object):
    ...
    def ensure_is_clean(self):
        ...
        if status:
            raise FatalError(
                "The GDB sandbox seems to be dirty (with"
    ...
    def has_branch(self, branch_name):
        ...
        try:
            self.git.show_ref("refs/heads/%s" % branch_name, verify=True, quiet=True)
            return True
        except CalledProcessError:
            return False

    def branch(self, branch_name, start_point):
        self.git.branch(branch_name, start_point, _outfile=sys.stdout)
        ...
        self.git.config(
            ...
            "refs/remotes/origin/%(branch_name)s" % {"branch_name": branch_name},
            add=True,
        )
    ...
    def delete_untracked_files(self):
        ...
        if untracked:
            warn(
                "Deleting the following untracked files from your sandbox:",
    ...
    def fetch_and_rebase(self, branch_name):
        trace("updating branch ${hl_sbu}%s${hl_ebu} in our sandbox..." % branch_name)
        self.ensure_is_clean()
        self.git.fetch(
            "origin",
            "+refs/heads/%(branch_name)s:"
        ...
        if not self.has_branch(branch_name):
            trace(
                "branch %s does not exist in your sandbox." % branch_name,
